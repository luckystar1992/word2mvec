...
/**
 * This is a modified version of word2vec published by Mikolov (Google).
 * I rewrite the Skip Gram model with Hierarcal Softmax algorithm.
 * Key point: word and last word. Because skip-gram is used to predict the context word
 *                   by a target-word, so we need to update target-word , not a context word.
 *                   An detail evaluation will be done in the evaluation floder with ws subfolder.
 */
